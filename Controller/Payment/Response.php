...
use Getnet\Payments\Actions\SetOrderInfoSession;
use Getnet\Payments\Constants\PathUrlRedirect;
...
class Response extends Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (strcmp($getnet->getCode(), PaymentMethod::CODE) !== 0) {
                throw new LocalizedException(__('Unknown payment method.'));
            }

            if (OrderHelper::isPendingOrder($order)) {
                $status = OrderHelper::getPaymentStatus($getnet->resolve($order, $payment));
            } else {
            ...
            $path = $this->scopeConfig->getValue(
                'payment/' . $getnet->getCode() . '/final_page',
            ...
            if (strcmp($path, 'magento_default') === 0) {
                if ($status->status() == Order::STATE_COMPLETE) {
                    $this->messageManager->addSuccessMessage(
                        sprintf(__('Thanks, transaction approved by')) . $getnet->getNameOfStore()
                    );
